...
StaticMesh::StaticMesh(HINSTANCE hInstance, const std::string& filePath)
	:D3DApp(hInstance)
{
	std::ifstream fin(filePath, std::ios::binary);
	meshInfo = DeserializeAssetInfo(fin);

	fin.close();
}

AssetInfo StaticMesh::DeserializeAssetInfo(std::ifstream& fin)
{
	AssetInfo assetInfo;
	...
	for (int i = 0; i < assetInfo.numLOD; i++)
	{
		...
		char* ch = (char*)meshesLODInfo.vertices.data();
		fin.read((char*)meshesLODInfo.vertices.data(), sizeof(FVector) * num);

		fin.read((char*)&num, sizeof(int32_t));
		meshesLODInfo.indices.resize(num);
		fin.read((char*)meshesLODInfo.indices.data(), sizeof(meshesLODInfo.indices) * num);
...
void StaticMesh::BuildConstantBuffers()
{
...
void StaticMesh::BuildRootSignature()
{
...
void StaticMesh::BuildShadersAndInputLayout()
{
	HRESULT hr = S_OK;
...
void StaticMesh::BuildBoxGeometry()
{
